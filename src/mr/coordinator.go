...
func (c *Coordinator) GetTask(
	...
	for mapID, m := range c.MapTasks {
		...
		if m.Status == TODO {
			...
			c.MapTasks[mapID].CreateTime = time.Now()
	...
	for reduceID, r := range c.ReduceTasks {
		if r.Status == TODO {
			...
			c.ReduceTasks[reduceID].CreateTime = time.Now()
...
func (c *Coordinator) server() {
	...
	go http.Serve(l, nil)
	// start a thread checks slow/unresponsive workers
...
func (c *Coordinator) checkTaskStatus() {
	for {
		...
		// Check the status of all map tasks.
		for i, mapTask := range c.MapTasks {
			if mapTask.Status == IN_PROGRESS && time.Since(mapTask.CreateTime) > 10*time.Second {
				c.MapTasks[i].Status = TODO
				log.Printf("🔙 Map task %d marked as TODO\n", mapTask.ID)
			}
		}

		// Check the status of all reduce tasks.
		for i, reduceTask := range c.ReduceTasks {
			if reduceTask.Status == IN_PROGRESS && time.Since(reduceTask.CreateTime) > 10*time.Second {
				c.ReduceTasks[i].Status = TODO
				log.Printf("🔙 Reduce task %d marked as TODO\n", reduceTask.ID)
			}
		}
